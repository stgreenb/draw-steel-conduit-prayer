...
Hooks.on('renderChatMessageHTML', (message, html) => {
  if (!message.content.includes('gains') || !message.content.includes('Piety')) return;

  for (const actor of game.actors.contents) {
    if (isConduit(actor) && actor.getFlag(CONDUIT_PRAYER.FLAG_SCOPE, CONDUIT_PRAYER.FLAG_HANDLING)) {
...
Hooks.once("ready", () => {
  ...
  if (!HeroModel?.prototype) return;

  setupSocketListener();
  const originalOnStartTurn = HeroModel.prototype._onStartTurn;
